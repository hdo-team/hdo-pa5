...
public class ImportSaveExecutable implements Executable<ImportModel> {
	private static Logger LOGGER = LogManager.getLogger(ImportSaveExecutable.class);
	...
	public void execute(ImportModel arg) {
		...
		try {
			...
			/**
			 * 
			 * Falls ein falsches Fileformat (kein .gz) ausgewaehlt wird, gibts
			 * hier die Exception und der Benutzer erhaelt einen Hinweis, dass
			 * der Importvorgang erfolgreich war...
			 * 
			 * Entweder bei der Fileauswahl checken ob es ein .gz Format ist,
			 * oder einen return-Value an die View mitgeben, damit dort dann
			 * dementsprechend der Dialog angezeigt wird.
			 * 
			 */
